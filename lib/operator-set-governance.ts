...
export function operatorSetRoot(set: OperatorSet) {
  ...
  uniq(set.members.map((m) => m.id), "Operator ids");
  uniq(set.members.map((m) => d64(m.credentialDigest, "Operator credential")), "Operator credentials");
  for (const m of set.members) {
    ...
    if (!Number.isInteger(m.activeFromSlot) || m.activeFromSlot < 1) throw new Error("Invalid operator activation slot");
    if (m.activeUntilSlot != null && m.activeUntilSlot <= m.activeFromSlot) throw new Error("Invalid operator expiry slot");
...
export function authorizeOperatorSetChange(previous: OperatorSet, change: OperatorSetChange, policy: OperatorSetPolicy, currentSlot: number) {
  ...
  if (change.previousRoot.toLowerCase() !== previousRoot) throw new Error("Operator predecessor root mismatch");
  if (change.next.epoch !== previous.epoch + 1) throw new Error("Operator epoch must advance exactly once");
