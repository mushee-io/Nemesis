...
function decimal(v: string, label: string) {
...
export function validateTestnetNotionalLifecycle(lifecycle: TestnetNotionalLifecycle, minimumConfirmations: number, nowMs = Date.now()) {
  ...
  if (lifecycle.fill.side === "BUY" && execution > limit) throw new Error("Notional BUY executed above committed limit");
  if (lifecycle.fill.side === "SELL" && execution < limit) throw new Error("Notional SELL executed below committed limit");
  ...
  const root = createHash("sha256").update([lifecycle.market,lifecycle.fill.side,limit,execution,lifecycle.fill.winningSolverId,lifecycle.fill.competingSolverCount,lifecycle.fill.solverFeeBps,chain.root].join("|")).digest("hex");
